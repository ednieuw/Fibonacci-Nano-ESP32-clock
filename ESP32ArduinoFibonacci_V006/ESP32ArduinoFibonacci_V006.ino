/* 
 ...
 Changes V006: Added WIFI router credentials login page. StartAPMode(), void ConnectWIFI(void)
                ...
                Removed sptext[0]=0; from (S)Tekstprint()
...
const char* AP_SSID = "StartFiboclock";
const char* AP_PASSWORD = "fiboclock";
...
void InitStorage(void)
{
 ...
 Mem.DisplayChoice   = _min(Mem.DisplayChoice, 9);                                            // Constrain the value to valid ranges 
...
{ 
 ...
 sprintf(sptext,"Display choice: %s",dp==0?"Mondriaan1":dp==1?"Mondriaan2":dp==2?"RGB":
...
// sprintf(sptext,"Password: %s", Mem.Password);                                                WTekstprintln(sptext);
 ...
 printf(sptext,"Software: %s",FILENAAM);                                                        WTekstprintln(sptext);  // VERSION);
...
//                                                                                            //
    ...
    case 'u':
            ...
            if (InputString.length() >1 && InputString.length() < 6 )                        // time between 0 and 999
              {
...
//--------------------------------------------
void Laatzien(int Delaymsec) { delay(Delaymsec);Laatzien(); }
void Laatzien()
{ 
 ShowLeds();
 LedsOff(); 
