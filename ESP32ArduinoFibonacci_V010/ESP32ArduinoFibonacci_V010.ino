/* 
 ...
 Changes V009: Bug in EdSoftLED. Removed
 Changes V010: Ontbrekende pin-definities voor ESP32-S3 DevKitC. Optimised buildHTML() and Tekstprintlnf & Tekstprintf.CheckMCURestartCounter ->10 min
...
const int NUM_LEDS = 17;    // How many leds in fibonacci clock? (12 / 14 /17 / 24 / 32 /36 /174 )
                            // check the LED positions in  setPixel() (at the end of this sketch) !!  
...
enum DigitalPinAssignments {
 ...
 encoderPinB  = 4,                 // D2 rotary CLK
 encoderPinA  = 5,                 // D3 rotary DT
 clearButton  = 6,                 // D4 rotary SW
 IRReceiverPin= 6,                 // D4 IR receiver (shared with clearButton)
 HC12_TX      = 17,                // GPIO17 HC-12 433MHz time sender 
 LED_PIN      = 48,                // GPIO48 SK6812/WS2812 LED strip
 encoderPinBL = 18,                // GPIO18 encoder alt pin (large PCB)
 PCB_LED_D09  = 10,                // D9
 PCB_LED_D10  = 11,                // D10
 secondsPin   = 13,                // D13
 };
...
{   
 ...
 CheckMCURestartCounter();                                                                    // MCUrestarted reset-window. Checks if startup went well 
 //GetTijd(false);
...
{ char Textbuf[256];
 va_list args; va_start(args, fmt); vsnprintf(Textbuf, sizeof(Textbuf), fmt, args); va_end(args);
 Tekstprint(Textbuf);
}
//--------------------------------------------                                                //
...
// snprintf(sptext, sizeof(sptext),"Password: %s", Mem.Password);                               WTekstprintln(sptext);
 ...
 if(!Small) {snprintf(ffftext,sizeof(ffftext),"%s", Mem.HC12Time?"HC-12Time=On":"HC-12Time=Off");} 
...
//--------------------------------------------                                                //
// COMMON Reset MCUrestart counter once we've been stable for 10 minutes
// Call from main loop. Cheap (millis check + one-shot flag).
//--------------------------------------------
void CheckMCURestartCounter(void)
{
 if (mcuRestartCounterCleared) return;                  // Already done this boot
 
 if (millis() < MCURESTART_RESET_AFTER_MS) return;      // Not yet stable long enough
 
 if (Mem.MCUrestarted != 0)
   {
    Tekstprintlnf("Stable for %u min, resetting MCUrestart counter (was %u)",
                  (unsigned)(MCURESTART_RESET_AFTER_MS / 60000),
                  (unsigned)Mem.MCUrestarted);
    Mem.MCUrestarted = 0;
    StoreStructInFlashMemory();
   }
 mcuRestartCounterCleared = true;
}
//--------------------------------------------                                                //
...
void BuildHTMLPage(bool Log)
{
 ...
 size_t topLen    = strlen(index_html_top);
 size_t infoLen   = strlen(html_info);
 size_t footerLen = strlen(index_html_footer);
 size_t totalLen  = topLen + infoLen + footerLen;
 
 if (totalLen >= MAXSIZE_HTML_PAGE)
   {
    snprintf(HTML_page, MAXSIZE_HTML_PAGE,
             "%s<br>Send I for menu\n*** INCREASE MAXSIZE_HTML_PAGE (need %u) ***<br>",
             index_html_top, (unsigned)totalLen);
    return;
   }
 
 memcpy(HTML_page + i, index_html_top,    topLen);     i += topLen;
 memcpy(HTML_page + i, html_info,         infoLen);    i += infoLen;
 memcpy(HTML_page + i, index_html_footer, footerLen);  i += footerLen;
 HTML_page[i] = 0;
}
